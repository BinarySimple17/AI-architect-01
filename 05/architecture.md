...
> **Примечание:** Конкретные названия продуктов (Kafka, Airflow, MinIO, Spark, Feast, Qdrant, MLflow и др.) приведены как примеры и не подразумевают реального сравнения или рекомендации.
...
**Консистентность Feature Store (обучение ↔ инференс):** Feast выступает единым источником истины для ML-признаков. Идентичная логика вычисления признаков через FeatureViews обеспечивается для offline (обучение) и online (инференс) путей, что предотвращает training-serving skew.

**Свежесть данных:** Стриминг-признаки (клик-счётчики) обновляются каждые 1-5 минут через Spark Streaming → Feast online. Батч-признаки (демография, категории) обновляются ежедневно через Spark batch → Feast offline. Векторные эмбеддинги пересчитываются ежедневно для товаров и в около real-time для пользователей.
