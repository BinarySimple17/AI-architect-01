# Архитектура системы «Умный помощник для поиска авиарейсов» (Travel Assistant)
...
|-----------|------------|
| **LLM‑провайдер** | Базовая языковая модель (OpenRouter с LLM моделью или локальная модель). Используется для извлечения параметров из запроса и генерации финального ответа. |
| **QueryParser** | Извлекает из пользовательского запроса город вылета, город прилёта и дату с помощью LLM и Pydantic‑парсера. |
| **FlightSearch** | Поиск авиарейсов в SQLite‑базе, построенной из CSV‑файла. Поддерживает прямые рейсы и поиск стыковочных маршрутов (одна пересадка). |
...
| **BudgetAnalyst** | Проверяет, укладывается ли хотя бы один из найденных рейсов в бюджетный лимит (по умолчанию 50 000 руб). |
| **FinalAnswerGenerator** | Формирует итоговый ответ пользователю, передавая LLM контекст из политики, список рейсов и результат бюджетной проверки. |
...
class AgentState(TypedDict):
    ...
    flights: List[Dict]       # список подходящих рейсов
...
```
query_parser  →  flight_searcher  →  policy_expert  →  reranker  →  budget_analyst  →  final_answer
```
...
```mermaid
graph TD
    A[query_parser<br/>Извлечение параметров] --> B[flight_searcher<br/>Поиск рейсов SQLite]
...
2. **FlightSearcher** использует `parsed_query` для поиска рейсов через `FlightSearch`. Результаты сохраняются в `flights`.  
...
RAG-пайплайн объединяет два источника данных: неструктурированную политику из векторного хранилища и структурированные результаты поиска рейсов из SQLite. Присутствует реранжинг чанков политики, повышающий качество контекста.
...
4. **Поиск рейсов**  
   Параллельно `FlightSearcher` по извлечённым из запроса городам и дате выполняет SQL‑запросы в SQLite (прямые и стыковочные рейсы). Результат — список словарей с информацией о рейсах и ценах — помещается в `flights`.
