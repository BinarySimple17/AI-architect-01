...
Сервис вырос до **10,000 RPS** при требовании **latency < 200ms** для синхронных запросов и **< 5s** для генерации LLM. Нижепредставленная архитектура масштабируется с учётом ограничений on-prem развёртывания и требований безопасности (запрет на внешние облака).
...
flowchart TD
    ...
    E --> F[Поиск в Qdrant<br/>requests + solution_chunks]
    F --> G[Ранжирование + LLM генерация]
    G --> H[Сохранение в Redis<br/>TTL 15 мин]
...
| TTL | 15 минут | Баланс актуальности и hit rate |
| Invalidation | TTL + ручная через Admin API | Обновление базы знаний требует очистки |
...
**Двухуровневый кэш:**
1. **Уровень 1:** Точный хэш (SHA256) → Redis lookup → O(1), < 10мс
2. **Уровень 2:** При промахе точного хэша — семантический поиск (эмбеддинг + Qdrant) → только если hit-rate семантического кэша > 70%
...
**Примечание:** Масштабируются только **поды приложения** (AI Service). GPU-ноды статичны (on-prem с фиксированным количеством GPU).
...
### Особенности масштабирования ML-сервисов
...
### RabbitMQ — конфигурация очередей

```
inference_queue:
  durability: durable
  prefetch_count: 32  # = max_batch_size для эффективного батчинга
  message_ttl: 300000  # 5 минут
  dead_letter_queue: dlq_inference

embedding_queue:
  durability: durable
  prefetch_count: 50  # Массовая обработка
  message_ttl: 600000  # 10 минут

etl_queue:
  durability: durable
  prefetch_count: 1
  message_ttl: 86400000  # 24 часа
```

**Примечание:** Батчинг (окно 0.5с) применяется **только в асинхронном пути**. Синхронный путь — без батчинга (одиночные запросы).
...
|----------|------------|------------|
| **GPU ресурсы** | ✅ Прямой доступ к nvidia-a100 | ❌ Нет поддержки GPU (Yandex) |
| **Stateful модели** | ✅ Долгоживущие поды с прогретыми моделями | ❌ Холодный старт 30-60с |
| **Latency** | ✅ < 200ms при pre-warmed | ❌ Холодный старт +3-5с |
...
**Вывод:** Для ML-инференса с требованиями latency < 200ms, on-prem развёртывания и политики безопасности Kubernetes — единственный вариант. Serverless подходит для ETL-задач (polling, embedding), но не для основного потока.

**Требования ИБ (из контекста системы):**
- Запрещена передача данных во внешние облака и публичные API
- mTLS между всеми внутренними контейнерами
- Шифрование данных в PostgreSQL и Qdrant
- Хранение секретов в HashiCorp Vault (уже развёрнут в компании)
- RBAC на Gateway для аутентификации и авторизации
...
**Semantic Cache** снижает нагрузку на LLM на **60-80%** для повторяющихся запросов. Ключевые особенности:

1. **Точный ключ хэша** — SHA256 от query + filters + config гарантирует, что одинаковые запросы с разными фильтрами не смешиваются
2. **TTL 15 минут** — баланс между актуальностью (база знаний обновляется раз в сутки) и hit rate
3. **Fallback приmiss** — при отсутствии в кэше запрос идёт полным потоком, результат сохраняется
4. **Инвалидация** — при обновлении базы знаний (ETL) автоматическая очистка затронутых ключей
...
- minReplicaCount=10 в рабочие часы для предотвращения холодного старта
...
| **Потребление** | Один consumer на сообщение | Consumer group с offset |
| **Latency** | ~мгновенная доставка | Буферизация в batch |
| **Ordering** | Не гарантирован (FIFO по очереди) | Гарантирован в partition |
| **Сложность** | Средняя | Высокая |
| **Производительность** | ~50k RPS | ~100k+ RPS |
...
## 6. Критерии оценки — соответствие

| Критерий | Статус | Комментарий |
|----------|--------|-------------|
| Semantic Cache снижает нагрузку на LLM | ✅ | 60-80% reduction для повторяющихся запросов |
| Масштабируемость для роста нагрузки | ✅ | KEDA autoscaling, min 3 → max 50 replicas |
| Latency < 200ms для sync запросов | ✅ | Cache hit path, multi-level caching |
| Кэш + Edge для снижения задержек | ✅ | Redis + KV Cache + CDN |
| Асинхронная обработка | ✅ | RabbitMQ queue для тяжёлых запросов |
| On-prem развёртывание | ✅ | K8s, все данные внутри периметра |
