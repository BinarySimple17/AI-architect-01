...
Все запросы поступают из ERP-системы, которая полностью расположена в Москве. Техподдержка работает с ERP — пользователи в других регионах все равно заходят через ERP. Распределение данных и инференса по регионам не даёт выгоды, но увеличивает стоимость инфраструктуры и сложность репликации.
